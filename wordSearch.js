...
const isWord = (x, y, direction, count = 0) => { 
    if (y >= lines.length || y < 0 || x >= lines[y].length || x < 0 || count > 3) return false
    if (lines[y][x] === word[count]) { 
        ...
        if (direction === 'diagLeftUp') return isWord(x - 1, y + 1, direction, count)
        if (direction === 'diagRightUp') return isWord(x + 1, y + 1, direction, count)
        if (direction === 'diagLeftDown') return isWord(x - 1, y - 1, direction, count)
        if (direction === 'diagRightDown') return isWord(x + 1, y - 1, direction, count)
    } else {
    return false
    }
}
